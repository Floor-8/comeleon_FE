...
(function (global, factory) {
  ...
  function ($, window, document, Math, undefined) {
    ...
    let WRAPPER = 'fullpage-wrapper';
    let WRAPPER_SEL = '.' + WRAPPER;

    // slimscroll
    let SCROLLABLE = 'fp-scrollable';
    let SCROLLABLE_SEL = '.' + SCROLLABLE;

    // util
    let RESPONSIVE = 'fp-responsive';
    let NO_TRANSITION = 'fp-notransition';
    let DESTROYED = 'fp-destroyed';
    let ENABLED = 'fp-enabled';
    let VIEWING_PREFIX = 'fp-viewing';
    let ACTIVE = 'active';
    let ACTIVE_SEL = '.' + ACTIVE;
    let COMPLETELY = 'fp-completely';
    let COMPLETELY_SEL = '.' + COMPLETELY;

    // section
    let SECTION_DEFAULT_SEL = '.section';
    let SECTION = 'fp-section';
    let SECTION_SEL = '.' + SECTION;
    let SECTION_ACTIVE_SEL = SECTION_SEL + ACTIVE_SEL;
    let SECTION_FIRST_SEL = SECTION_SEL + ':first';
    let SECTION_LAST_SEL = SECTION_SEL + ':last';
    let TABLE_CELL = 'fp-tableCell';
    let TABLE_CELL_SEL = '.' + TABLE_CELL;
    let AUTO_HEIGHT = 'fp-auto-height';
    let AUTO_HEIGHT_SEL = '.fp-auto-height';
    let NORMAL_SCROLL = 'fp-normal-scroll';
    let NORMAL_SCROLL_SEL = '.fp-normal-scroll';

    // section nav
    let SECTION_NAV = 'fp-nav';
    let SECTION_NAV_SEL = '#' + SECTION_NAV;
    let SECTION_NAV_TOOLTIP = 'fp-tooltip';
    let SECTION_NAV_TOOLTIP_SEL = '.' + SECTION_NAV_TOOLTIP;
    let SHOW_ACTIVE_TOOLTIP = 'fp-show-active';

    // slide
    let SLIDE_DEFAULT_SEL = '.slide';
    let SLIDE = 'fp-slide';
    let SLIDE_SEL = '.' + SLIDE;
    let SLIDE_ACTIVE_SEL = SLIDE_SEL + ACTIVE_SEL;
    let SLIDES_WRAPPER = 'fp-slides';
    let SLIDES_WRAPPER_SEL = '.' + SLIDES_WRAPPER;
    let SLIDES_CONTAINER = 'fp-slidesContainer';
    let SLIDES_CONTAINER_SEL = '.' + SLIDES_CONTAINER;
    let TABLE = 'fp-table';

    // slide nav
    let SLIDES_NAV = 'fp-slidesNav';
    let SLIDES_NAV_SEL = '.' + SLIDES_NAV;
    let SLIDES_NAV_LINK_SEL = SLIDES_NAV_SEL + ' a';
    let SLIDES_ARROW = 'fp-controlArrow';
    let SLIDES_ARROW_SEL = '.' + SLIDES_ARROW;
    let SLIDES_PREV = 'fp-prev';
    let SLIDES_PREV_SEL = '.' + SLIDES_PREV;
    let SLIDES_ARROW_PREV = SLIDES_ARROW + ' ' + SLIDES_PREV;
    let SLIDES_ARROW_PREV_SEL = SLIDES_ARROW_SEL + SLIDES_PREV_SEL;
    let SLIDES_NEXT = 'fp-next';
    let SLIDES_NEXT_SEL = '.' + SLIDES_NEXT;
    let SLIDES_ARROW_NEXT = SLIDES_ARROW + ' ' + SLIDES_NEXT;
    let SLIDES_ARROW_NEXT_SEL = SLIDES_ARROW_SEL + SLIDES_NEXT_SEL;
    ...
    $.fn.fullpage = function (options) {
      ...
      let scrollId2;
      let keydownId;
      let originals = $.extend(true, {}, options); //deep copy
      ...
      function setAutoScrolling(value, type) {
        ...
        let element = $(SECTION_ACTIVE_SEL);
      ...
      function moveSectionUp() {
        let prev = $(SECTION_ACTIVE_SEL).prev(SECTION_SEL);
      ...
      function moveSectionDown() {
        let next = $(SECTION_ACTIVE_SEL).next(SECTION_SEL);
      ...
      function moveTo(sectionAnchor, slideAnchor) {
        let destiny = getSectionByAnchor(sectionAnchor);
      ...
      function reBuild(resizing) {
        ...
        $(SECTION_SEL).each(function () {
          let slidesWrap = $(this).find(SLIDES_WRAPPER_SEL);
          let slides = $(this).find(SLIDE_SEL);
        ...
        let activeSection = $(SECTION_ACTIVE_SEL);
        let sectionIndex = activeSection.index(SECTION_SEL);
      ...
      function setOptionsFromDOM() {
        let sections = container.find(options.sectionSelector);
      ...
      function prepareDom() {
        ...
        $(SECTION_SEL).each(function (index) {
          let section = $(this);
          let slides = section.find(SLIDE_SEL);
      ...
      function styleSlides(section, slides, numSlides) {
        let sliderWidth = numSlides * 100;
        let slideWidth = 100 / numSlides;
        ...
        let startingSlide = section.find(SLIDE_ACTIVE_SEL);
      ...
      function addVerticalNavigation() {
        ...
        for (let i = 0; i < $(SECTION_SEL).length; i++) {
          ...
          let li = '<li><a href="#' + link + '"><span></span></a>';

          // Only add tooltip if needed (defined by user)
          let tooltip = options.navigationTooltips[i];
      ...
      function afterRenderActions() {
        let section = $(SECTION_ACTIVE_SEL);
      ...
      function isDestinyTheStartingSection() {
        let destinationSection = getSectionByAnchor(getAnchorsURL().section);
        return (
          !destinationSection ||
      ...
      function scrollHandler() {
        ...
        if (!options.autoScrolling || options.scrollBar) {
          ...
          let screen_mid = currentScroll + $window.height() / 2.0;
          let isAtBottom = $body.height() - $window.height() === currentScroll;
          let sections = document.querySelectorAll(SECTION_SEL);
          ...
          if (!currentSection.hasClass(ACTIVE)) {
            isScrolling = true;
            let leavingSection = $(SECTION_ACTIVE_SEL);
            let leavingSectionIndex = leavingSection.index(SECTION_SEL) + 1;
            let yMovement = getYmovement(currentSection);
            let anchorLink = currentSection.data('anchor');
            let sectionIndex = currentSection.index(SECTION_SEL) + 1;
            let activeSlide = currentSection.find(SLIDE_ACTIVE_SEL);
            let slideIndex;
            let slideAnchorLink;
      ...
      function isCompletelyInViewPort(movement) {
        let top = $(SECTION_ACTIVE_SEL).position().top;
      ...
      function scrolling(type) {
        ...
        let scrollSection = type === 'down' ? moveSectionDown : moveSectionUp;

        if (options.scrollOverflow) {
          let scrollable = options.scrollOverflowHandler.scrollable(
            $(SECTION_ACTIVE_SEL)
          );
      ...
      */
      function touchMoveHandler(event) {
        let e = event.originalEvent;
        let activeSection = $(e.target).closest(SECTION_SEL);
      ...
      function MouseWheelHandler(e) {
        ...
        let isNormalScroll = $(COMPLETELY_SEL).hasClass(NORMAL_SCROLL);
      ...
      function moveSlide(direction, section) {
        ...
        let slides = activeSection.find(SLIDES_WRAPPER_SEL);
        let numSlides = slides.find(SLIDE_SEL).length;
        ...
        let currentSlide = slides.find(SLIDE_ACTIVE_SEL);
        let destiny = null;
      ...
      function getDestinationPosition(element) {
        ...
        let position = elemPosition.top;
        let isScrollingDown = elemPosition.top > previousDestTop;
        let sectionBottom = position - windowsHeight + element.outerHeight();
        let bigSectionsDestination = options.bigSectionsDestination;
      ...
      function scrollPage(element, callback, isMovementUp) {
        ...
        let dtop = getDestinationPosition(element);
        let slideAnchorLink;
        let slideIndex;
      ...
      function performMovement(v) {
        ...
        // using jQuery animate
          ...
          let scrollSettings = getScrollSettings(v);
        ...
        panel
          ...
          .each(function () {
            ...
            $.each(['src', 'srcset'], function (index, type) {
              let attribute = element.attr('data-' + type);
            ...
            if (element.is('source')) {
              let typeToPlay = element.closest('video').length
                ? 'video'
                : 'audio';
      ...
      function getSlideOrSection(destiny) {
        let slide = destiny.find(SLIDE_ACTIVE_SEL);
        if (slide.length) {
          destiny = $(slide);
        }
      ...
      function scrollToAnchor() {
        let anchors = getAnchorsURL();
        let sectionAnchor = anchors.section;
        let slideAnchor = anchors.slide;
      ...
      function hashChangeHandler() {
        if (!isScrolling && !options.lockAnchors) {
          ...
          let isFirstSlideMove = typeof lastScrolledDestiny === 'undefined';
          let isFirstScrollMove =
      ...
      function getAnchorsURL() {
        ...
        if (hash.length) {
          ...
          let anchorsParts = hash.replace('#', '').split('/');

          //using / for visual reasons and not as a section/slide separator #2803
          let isFunkyAnchor = hash.indexOf('#/') > -1;

          section = isFunkyAnchor
            ? '/' + anchorsParts[1]
            : decodeURIComponent(anchorsParts[0]);

          let slideAnchor = isFunkyAnchor ? anchorsParts[2] : anchorsParts[1];
      ...
      function slideArrowHandler() {
        let section = $(this).closest(SECTION_SEL);
      ...
      function slideBulletHandler(e) {
        e.preventDefault();
        let slides = $(this).closest(SECTION_SEL).find(SLIDES_WRAPPER_SEL);
        let destiny = slides.find(SLIDE_SEL).eq($(this).closest('li').index());
      ...
      function onTab(e) {
        ...
        let activeElement = $(':focus');
        let activeSection = $(SECTION_ACTIVE_SEL);
        let activeSlide = activeSection.find(SLIDE_ACTIVE_SEL);
        let focusableWrapper = activeSlide.length ? activeSlide : activeSection;
        let focusableElements = focusableWrapper
      ...
      function landscapeScroll(slides, destiny, direction) {
        let section = slides.closest(SECTION_SEL);
        let v = {
          slides: slides,
          destiny: destiny,
          direction: direction,
       ...
       */
      function responsive() {
        let widthLimit = options.responsive || options.responsiveWidth; //backwards compatiblity
        let heightLimit = options.responsiveHeight;

        //only calculating what we need. Remember its called on the resize event.
        let isBreakingPointWidth =
          widthLimit && $window.outerWidth() < widthLimit;
        let isBreakingPointHeight =
      ...
      function getYmovement(destiny) {
        let fromIndex = $(SECTION_ACTIVE_SEL).index(SECTION_SEL);
        let toIndex = destiny.index(SECTION_SEL);
      ...
      function addTableClass(element) {
        ...
        if (!element.hasClass(TABLE)) {
          let wrapper = $('<div class="' + TABLE_CELL + '" />').height(
      ...
      function scrollPageAndSlide(sectionAnchor, slideAnchor) {
        let section = getSectionByAnchor(sectionAnchor);

        //do nothing if there's no section with the given anchor name
        if (!section.length) return;

        let slide = getSlideByAnchor(slideAnchor, section);
      ...
      function addSlidesNavigation(section, numSlides) {
        ...
        let nav = section.find(SLIDES_NAV_SEL);
      ...
      function setBodyClass() {
        let section = $(SECTION_ACTIVE_SEL);
        let slide = section.find(SLIDE_ACTIVE_SEL);

        let sectionAnchor = getAnchor(section);
        let slideAnchor = getAnchor(slide);
      ...
      function silentScroll(top) {
        // The first section can have a negative value in iOS 10. Not quite sure why: -0.0142822265625
        // that's why we round it to 0.
        let roundedTop = Math.round(top);

        if (options.css3 && options.autoScrolling && !options.scrollBar) {
          let translate3d = 'translate3d(0px, -' + roundedTop + 'px, 0px)';
      ...
      function destroyStructure() {
        ...
        //removing selectors
        let usedSelectors = [SECTION, SLIDE, SLIDES_CONTAINER];
      ...
      function displayWarnings() {
        ...
        $.each(options.anchors, function (index, name) {
          ...
          let nameAttr = $document.find('[name]').filter(function () {
            return (
              $(this).attr('name') &&
          ...
          let idAttr = $document.find('[id]').filter(function () {
            return (
              $(this).attr('id') &&
